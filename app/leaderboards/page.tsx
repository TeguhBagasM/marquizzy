...
const page = async () => {
  ...
  return (
    <div className="max-w-[1500px] mx-auto w-[90%] py-10">
      <h1 className="font-bold mb-4 text-center text-2xl uppercase">Leaderboards 🏆</h1>
      <ol>
        {users.map((user, index) => (
          <li key={user.id} className={`py-4 ${index < 3 ? "font-bold" : ""}`}>
            <div className="flex items-center gap-5 w-full">
              <div className="flex sm:flex-row flex-col gap-1 justify-between w-full items-center">
                <div className="flex gap-3 items-center">
                  ...
                  {index === 0 && <FaCrown className="inline-block w-6 h-6 text-yellow-500 mb-1" />}
                </div>
                <span>
